...
<div class="panel-heading" >
            <a href="{{route('purchase.ordercs')}}" class="btn btn-success btn-alt btn-xs" style="border-radius: 0px !important;" >
...
<div class="panel-body">
	{{ csrf_field() }}
	<h3 class="title-hero"> Editar Orden de Compra</h3>
			    ...
			    <div class="col-md-5">	
			      <select class="form-control selectpicker" data-live-search="true">
			      	@foreach($suppliers as $supplier)
			      	    @if($supplier->id == $orderc->client_id)
				 ...
				 @foreach($ordercds as $item)
					<tr class="item{{$item->id}}">
                          ...
                          <td class="text-center">
							...
							data-info="{{ $item->id }},{{ $item->name }},{{ $item->cost_price }},{{ $item->quantity }},{{ $item->subtotal }}">
    ...
    <div id="app">
    	<div>
    		<input type="hidden" id="norden" value="{{$orderc->id}}">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form class="form-horizontal" role="form">
						...
						<div class="form-group">
							<label class="control-label col-sm-2" for="price">Unit Cost:</label>
							<div class="col-sm-10">
								<input type="name" class="form-control" id="cost_price" readonly>
							</div>
						</div>
						<div class="form-group">
...
@section('js')
	...
	<script>
		...
		function fillmodalData(details){
		    $('#fid').val(details[0]);
		    $('#name').val(details[1]);
		    $('#cost_price').val(details[2]);
		    $('#quantity').val(details[3]);
		    $('#subtotal').val(details[4]);
		}

		$('.modal-footer').on('click', '.edit', function() {
			    subtotal = $('#cost_price').val() *  $('#quantity').val(); 
		        $.ajax({
		            type: 'post',
		            url: '/admin/ordercd/update',
		            data: {
		            ...
		            success: function(data) {
		                ...
		                $('.item' + data.id).replaceWith("<tr class='item" + data.id + "'><td>" +
		                        ...
		                        "</td><td>" + data.cost_price + "</td><td>" + data.quantity + "</td><td>" +
		                         data.subtotal + "</td><td><button class='edit-modal btn btn-info' data-info='" + data.id+","+data.name+","+data.cost_price+","+data.quantity+","+data.subtotal+"'><span class='glyphicon glyphicon-edit'></span></button> <button class='delete-modal btn btn-danger' data-info='" + data.id+","+data.name+","+data.cost_price+","+data.quantity+","+data.subtotal+"' ><span class='glyphicon glyphicon-trash'></span> </button></td></tr>");
    	...
    	var app = new Vue({
		    ...
		    methods:{
		        ...
		        postForm: function () {
		        	...
		        	var self = this
					axios.post('/admin/addorden', { purchases: this.purchases, norden: norden })
